...
\item[{[G8]}] The maximum time to accept an individual request from any third party is 30 days; after that, the request will expire
	\begin{itemize}
	\item[{[R20]}] Once the time elapsed from sending request is greater than 30 days, then the request will be deleted from the system
	\end{itemize}
\item[{[G9 \& G10]}] Allow a user to accept or refuse a request from third parties. Allow a user to block requests made by a specific third party
	\begin{itemize}
	\item[{[R21]}] Allow a user to receive individual requests about data sharing from third parties
	\item[{[R22]}] Allow a user to view a list of pending requests
	\item[{[R23]}] Allow a user to accept or refuse a request from the list of pending request
	\item[{[R24]}] Allow a user to block requests from a defined third party, after refusing it
	\end{itemize}	
\item[{[G13]}] In case it is impossible to start a run, the enrolled runners are notified
	\begin{itemize}
	\item[{[R25]}] If, after the expiration date, the number of participants is less than the minimum number defined by the organizer, then it is impossible to start the run.
	\item[{[R26]}] If the run cannot start due to minimum number of participants unsatisfied, then the enrolled runners are notified.
	\end{itemize}
\item[{G12}] Allow spectators and runners to see the leaderboard, when a run is completed
	...
	\item[{R27}] Allow an organizer to close the run (when it is terminated)
	\item[{R28}] After a run is closed, show the leaderboard to the spectators and runners
	\end{itemize}
...
\item[{[G15]}] Allow a third party to subscribe to non-existing data. They will have access to them, as soon as the data is generated. 
	\begin{itemize}
	\item[{[R37]}] Allow a third party to express a data request on future data
	\item[{[R38]}] A third party can have access to non-existing aggregated data regarding future information if and only if the number of people involved will be greater than 1000
	\end{itemize}
\end{itemize}
